...
try {
    ...
    $req = $pdo->prepare("SELECT nom_fichier, chemin_fichier FROM fichiers WHERE id_utilisateur = ? ORDER BY id DESC");#requête préparée avec marqueur pour récupérer tous les fichiers de l'utilisateur connecté
    $req->execute([$id_user]);
